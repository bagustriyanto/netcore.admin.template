...
namespace SarayaAdmin.Service.Services {
    public class RoleServices : IRoleServices {
        ...
        public BaseResponse<Role> DeleteList (long[] id) {
            BaseResponse<Role> result = new BaseResponse<Role> ();
            Expression<Func<Role, bool>> predicate = null;

            try {
                foreach (var itemId in id) {
                    predicate = x => x.Id.Equals (itemId);
                    var role = _unitOfwork.GetRepository<Role> ().Single (predicate);
                    if (role == null)
                        throw new Exception ("ERROR-9998");

                    _unitOfwork.GetRepository<Role> ().Delete (itemId);
                }
